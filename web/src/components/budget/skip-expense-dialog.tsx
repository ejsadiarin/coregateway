...
export function SkipExpenseDialog({ expense, open, onOpenChange }: SkipExpenseDialogProps) {
  ...
  const handleOpen = (isOpen: boolean) => {
    if (isOpen && expense) {
      setSkipDate(expense.next_due_date);
      setReason('');
      setAlreadySkipped(false);
    }
    onOpenChange(isOpen);
        ...
        <div className="space-y-4 py-4">
          <div className="p-4 rounded-lg bg-muted space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium capitalize">{expense.recurring_type || 'N/A'}</span>
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="skip-expense-date">Skip Date</Label>
            <Input
              id="skip-expense-date"
              type="date"
              value={skipDate}
              onChange={(e) => {
          ...
          <div className="space-y-2">
            <Label htmlFor="skip-expense-reason">Reason (optional)</Label>
            <Textarea
              id="skip-expense-reason"
              placeholder="e.g., service paused, free trial month..."
              value={reason}
